                            ...
                            <div class="draw-box d-flex profile-card m-0">
                                <div class="controls">
                                    ...
                                    <div class='lightBorder'>
                                        ...
                                        <p><input type="radio" name="shape" value="fountainpen" id="fountainpen"> Fountain Pen</p>
                                        <p><input type="radio" name="shape" value="shadow" id="shadow"> Shadow Brush </p>
                                ...
                                <input class=" d-none" id="uploader">
                                    ...
                                    <div id="xordiv" class='lightBorder'>
...
function drawSquare(position) {
    ...
    freehandRadio.addEventListener('change', handleFreehandChange);
    ...
    //----------------------------------------------------------------------------------------------------------------------------------------
    const fountainPenButton = document.querySelector('input[type="radio"][name="shape"][value="fountainpen"]');
fountainPenButton.addEventListener("mousedown", function () {
    // Call a function to activate the Fountain Pen tool
    activateFountainPen();
});
...
function endFountainPen() {
    ...
    fountainpen.checked = false;

    // Perform any necessary cleanup when the Fountain Pen tool is deactivated
}
...
var sprayButtonClicked = false; 
 ...
 // document.getElementById('sprayPaintButton').addEventListener('click', toggleSprayPaint);
 ...
 sprayPaintButton.addEventListener("click", function () {
    sprayButtonClicked = true;
    
    sprayPaintButton.checked = true;
...
function toggleSprayPaint() {
  if (sprayButtonClicked) {
...
function sprayPaint(event) {
  if (sprayPainting) {
    var position = getCanvasCoordinates(event);
    for (var i = 0; i < 30; i++) { // Adjust the number of paint particles as needed
...
function endSprayPaint() {
  sprayPainting = false;
  const sprayPaintButton = document.querySelector('input[type="radio"][name="shape"][value="sprayPaintButton"]');
  sprayPaintButton.checked = false;
}
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    fountainpen.addEventListener('click', () => {
     ...
     //-------------------------------------------------------------------------------------------------
     const frsprayPaintButtoneehand = document.querySelector('input[type="radio"][name="shape"][value="sprayPaintButton"]');
...
// Make sure the image is loaded before performing any actions with it
img.onload = function () {
    init(); // Call your init() function after the image is loaded
};
...
function startDraggingImage(event) {
    ...
    if (
        ...
        // Calculate the offset between the mouse click and the image's top-left corner
        imageOffsetX = mouseX - imageX;
        imageOffsetY = mouseY - imageY;

        console.log("Dragging Image:", isDraggingImage);
        console.log("OffsetX:", imageOffsetX);
        console.log("OffsetY:", imageOffsetY);
    }
}
...
function dragImage(event) {
    if (isDraggingImage) {
        // Calculate the new image position based on mouse coordinates and offset
        const mouseX = event.clientX - canvas.getBoundingClientRect().left;
        const mouseY = event.clientY - canvas.getBoundingClientRect().top;
        imageX = mouseX - imageOffsetX;
        imageY = mouseY - imageOffsetY;

        // Redraw the canvas, including the image at the new position
        redrawCanvas();
    }
}
...
function redrawCanvas() {
    ...
    // Redraw the uploaded image at the updated position
 ...
 // Offset between mouse click and image's top-left corner
function startDraggingImage(event) {
    const mouseX = event.clientX - canvas.getBoundingClientRect().left;
    const mouseY = event.clientY - canvas.getBoundingClientRect().top;
...
canvas.addEventListener('click', endFountainPen);
     //-----------
      ...
      //  canvas.addEventListener('mousedown', startSprayPaint);
    ...
    // Uncheck the "Freehand" radio button
    freehandRadio.checked = false;
    
}, false);

canvas.addEventListener('mousedown', startDraggingImage, false);
canvas.addEventListener('mousemove', dragImage, false);
canvas.addEventListener('mouseup', stopDraggingImage, false);
